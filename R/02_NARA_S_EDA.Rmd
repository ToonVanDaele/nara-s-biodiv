...
We gaan ervan uit dat de standplaatsvariabelen "potnattrofie" en "drainage"
gebonden zijn aan het landgebruik. Voor drainage lijkt dit niet het geval.
We behouden daarom drainage en beschouwen die constant over de 
verschillende scenarios. Potnattrofie wordt niet verder gebruikt in de analyse.
...
```{r standardisation}
# Calculate scaling values based on the values for the current scenario
expl_scale <- df_expl %>%
  filter(scen == "Current" & !varname == "Opp_m2") %>%
  group_by(varname) %>%
  summarise(smean = mean(value),
            ssd = sd(value))

# Apply the same standardisation for all scenarios
df_expl <- df_expl %>%
  left_join(expl_scale,
            by = "varname") %>%
  mutate(svalue = (value - smean) / ssd) %>%
  dplyr::select(-smean, -ssd)
