...
a <- df_expl %>%
  ...
  mutate(opp_ok = ifelse(value > 700000, 1, 0))

ggplot(a, aes(x = X, y = Y, colour = factor(opp_ok))) + 
...
speclist <- df_plant %>%
  group_by(Taxoncode) %>%
  count() %>%
  filter(n >= 100 & n <= 0.95 * nb_utm) %>%
  pull(Taxoncode)
```

A total of 'r count(speclist)' species.
...
spr_plant <- inner_join(x = select(a, utmID, X, Y), 
                        y = spr_plant,
                        by = "utmID")
...
# Selectie van de (mogelijk) relevante verklarende variabelen per soort

- Maximaal aantal variabelen = min(presences & absences) / 10
- Minstens 20% van waarden > 0 (voor landgebruiksklassen)
- VIF <- 5

Aantal variabelen per species

```{r selectvars}
g <- map_dfr(colnames(spr_plant[-c(1:3)]), getexplvar,
             spr_plant = spr_plant,
             df_expl = df_expl)
...
# Filter op basis minstens 5 % hokken met waarde > 0 en 
#minimaal 5 distincte waarden

explvar1 <- g %>%
 filter(distinct > 5)
#filter(count / nbhokken > 0.05 & distinct > 1)
...
# Verdere selectie op basis van VIF

```{r VIF}
speclist <- unique(explvar1$sp_n)

dfVIF <- map_dfr(speclist, VIFselect, explvar1, spr_plant, df_expl)

dfVIF %>%
  group_by(varname) %>%
  summarize(meanVIF = mean(VIF),
            medianVIF = median(VIF),
            maxVIF = max(VIF),
            minVIF = min(VIF))
```


```{r selectvar, results='asis'}
explspec <- inner_join(x = explvar1,
                      y = dfVIF,
                      by = c('sp_n', 'varname'))

explspec %>%
  group_by(sp_n) %>%
  summarise(maxnbvar = first(maxnbvar),
            nbvars = n()) %>%
  mutate(ok = ifelse(maxnbvar < nbvars, "TRUE", "FALSE"))
...
Meest gekozen variabelen

```{r varlist, results='asis'}
dfVIF %>%
  group_by(varname) %>%
  count() %>%
  pandoc.table()
...
#remove attributes  
#df_expl_sc <- as.data.frame(lapply(df_expl_sc, FUN = as.vector))  
#df_expl_sc <- bind_cols(df_expl %>%
#            select(utmID, X, Y, scen),
#            df_expl_sc)
```
