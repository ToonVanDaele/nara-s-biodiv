...
make_rank_plot <- function(df) {
  ...
  p <- df %>%
    ...
    select(Actueel = rlw_Current, CI = rlw_kr1, NW = rlw_kr2, SE = rlw_kr3, SN = rlw_kr4) %>%
    gather(key = "scenario", value = "aantal") %>%
    group_by(scenario) %>%
    arrange(scenario, aantal) %>%
    mutate(percent = row_number() / sample_size * 100) %>%
    ggplot(aes(x = percent, y = aantal, colour = scenario)) + geom_path() +
    scale_colour_manual(values = c("black", "green2", "orange", "deeppink2", "blue"))
}
```
